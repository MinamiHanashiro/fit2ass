...
flag = true;
  ...
  charactor.image = game.assets['images/meid_black_glad.png'];
	...
	//ラベルの生成
	scene.addChild(makeMessage("しゅり:なんだか暇してそうだったから話しかけてみたの！じゃあ私が首里城を紹介してあげる！"));
  var select0 = makeSelect("【次へ】", 10, 320 - 12 * 2);
	select0.addEventListener(Event.TOUCH_START, function(e) {
...
//ラベルの生成
scene.addChild(makeMessage("しゅり:じゃあ〜早速！首里城は、14世紀末頃に建設されたと言われる琉球王朝のお城なんだよ。"));
//選択肢の生成
...
//ラベルの生成
scene.addChild(makeMessage("しゅり:国王とその家族が住んでいたと言われているの。でもね、実は首里城ってこれまでに4回も焼失しちゃったの…"));
//選択肢の生成
var select0 = makeSelect("【次へ】", 10, 320 - 12 * 2);
select0.addEventListener(Event.TOUCH_START, function(e) {
...
//ラベルの生成
scene.addChild(makeMessage("しゅり:現在残っているものは、ほとんどが復元されたものなんだよ。ちなみに首里城は、中国と日本の文化を融合した独特の建築様式なんだ…！"));
//選択肢の生成
var select0 = makeSelect("【次へ】", 10, 320 - 12 * 2);
select0.addEventListener(Event.TOUCH_START, function(e) {
